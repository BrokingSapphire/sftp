...
import { Inbox, Download, Trash2, Check, Eye, ShieldAlert } from "lucide-react";
...
export default function InheritedPage() {
  ...
  async function keepAll() {
    try { await Promise.all(files.map((f) => filesApi.keepFile(f.id))); toast.success("All files kept"); refresh(); }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <PageHeader title="Inherited files" subtitle="Files assigned to you from a removed account" />
        {files.length > 0 && <Button size="sm" onClick={keepAll}><Check size={16} /> Keep all</Button>}
      </div>
      ...
      {files.length > 0 && (
        <div className="flex items-start gap-2 rounded-xl border border-warning/40 bg-warning/10 px-4 py-3 text-sm text-warning">
          ...
          <p>
            Review each file below — <strong>keep</strong> it in your drive or <strong>delete</strong> it.
            Take action before the deadline or your account may be disabled. Nothing is deleted automatically.
          </p>
        </div>
      )}
      ...
      {!q.isLoading && files.length === 0 && (
        <div className="flex min-h-[18rem] flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-surface text-muted">
          <Inbox size={40} />
          <p className="text-sm">No inherited files. You&apos;re all caught up.</p>
        </div>
      )}

      {files.length > 0 && (
        <div className="rounded-xl border border-border bg-surface">
          <div className="grid grid-cols-[1fr_auto_7rem_auto] gap-4 border-b border-border px-4 py-2.5 text-xs font-medium uppercase tracking-wider text-muted">
            <span>Name</span><span>Size</span><span>Deadline</span><span className="text-right">Action</span>
          </div>
          <StaggerList>
            {files.map((f, i) => {
              const dl = deadlineLabel(f.transfer_deadline);
              return (
                <StaggerItem key={f.id} className="grid grid-cols-[1fr_auto_7rem_auto] items-center gap-4 border-b border-border/50 px-4 py-2.5 transition-colors hover:bg-surface-2">
