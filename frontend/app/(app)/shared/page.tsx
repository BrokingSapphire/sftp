    ...
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      {files.length > 0 && (
        <div className="rounded-xl border border-border bg-surface">
          <div className="grid grid-cols-[1fr_11rem_5rem_auto_7rem] gap-4 border-b border-border px-4 py-2.5 text-xs font-medium uppercase tracking-wider text-muted">
            <span>Name</span><span>Owner</span><span>Access</span><span>Size</span><span className="text-right">Shared</span>
          </div>
          <StaggerList>
            {files.map((f, i) => (
              <StaggerItem key={f.id} className="group grid grid-cols-[1fr_11rem_5rem_auto_7rem] items-center gap-4 border-b border-border/50 px-4 py-2.5 transition-colors hover:bg-surface-2">
